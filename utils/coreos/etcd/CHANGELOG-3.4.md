...
Previous change logs can be found at [CHANGELOG-3.3](https://GDTS/utils/coreos/etcd/blob/master/CHANGELOG-3.3.md).


## v3.4.0 (TBD 2018-09)

See [code changes](https://GDTS/utils/coreos/etcd/compare/v3.3.0...v3.4.0) and [v3.4 upgrade guide](https://GDTS/utils/coreos/etcd/blob/master/Documentation/upgrades/upgrade_3_4.md) for any breaking changes. **Again, before running upgrades from any previous release, please make sure to read change logs below and [v3.4 upgrade guide](https://GDTS/utils/coreos/etcd/blob/master/Documentation/upgrades/upgrade_3_4.md).**

### Improved

- Rewrite [client balancer](https://GDTS/utils/coreos/etcd/pull/9860) with [new gRPC balancer interface](https://GDTS/utils/coreos/etcd/issues/9106).
- Add [backoff on watch retries on transient errors](https://GDTS/utils/coreos/etcd/pull/9840).
- Add [jitter to watch progress notify](https://GDTS/utils/coreos/etcd/pull/9278) to prevent [spikes in `etcd_network_client_grpc_sent_bytes_total`](https://GDTS/utils/coreos/etcd/issues/9246).
...
- Improve [TLS setup error logging](https://GDTS/utils/coreos/etcd/pull/9518) to help debug [TLS-enabled cluster configuring issues](https://GDTS/utils/coreos/etcd/issues/9400).
- Improve [long-running concurrent read transactions under light write workloads](https://GDTS/utils/coreos/etcd/pull/9296).
...
- Add [Raft Pre-Vote feature](https://GDTS/utils/coreos/etcd/pull/9352) to reduce [disruptive rejoining servers](https://GDTS/utils/coreos/etcd/issues/9333).
...
- Improve [lease expire/revoke operation performance](https://GDTS/utils/coreos/etcd/pull/9418), address [lease scalability issue](https://GDTS/utils/coreos/etcd/issues/9496).
- Make [Lease `Lookup` non-blocking with concurrent `Grant`/`Revoke`](https://GDTS/utils/coreos/etcd/pull/9229).
- Make etcd server return `raft.ErrProposalDropped` on internal Raft proposal drop in [v3 applier](https://GDTS/utils/coreos/etcd/pull/9549) and [v2 applier](https://GDTS/utils/coreos/etcd/pull/9558).
  - e.g. a node is removed from cluster, or [`raftpb.MsgProp` arrives at current leader while there is an ongoing leadership transfer](https://GDTS/utils/coreos/etcd/issues/8975).
- Add [`snapshot`](https://GDTS/utils/coreos/etcd/pull/9118) package for easier snapshot workflow (see [`godoc.org/github.com/etcd/clientv3/snapshot`](https://godoc.org/GDTS/utils/coreos/etcd/clientv3/snapshot) for more).
- Improve [functional tester](https://GDTS/utils/coreos/etcd/tree/master/functional) coverage: [proxy layer to run network fault tests in CI](https://GDTS/utils/coreos/etcd/pull/9081), [TLS is enabled both for server and client](https://GDTS/utils/coreos/etcd/pull/9534), [liveness mode](https://GDTS/utils/coreos/etcd/issues/9230), [shuffle test sequence](https://GDTS/utils/coreos/etcd/issues/9381), [membership reconfiguration failure cases](https://GDTS/utils/coreos/etcd/pull/9564), [disastrous quorum loss and snapshot recover from a seed member](https://GDTS/utils/coreos/etcd/pull/9565), [embedded etcd](https://GDTS/utils/coreos/etcd/pull/9572).
- Improve [index compaction blocking](https://GDTS/utils/coreos/etcd/pull/9511) by using a copy on write clone to avoid holding the lock for the traversal of the entire index.
- Update [JWT methods](https://GDTS/utils/coreos/etcd/pull/9883) to allow for use of any supported signature method/algorithm.

### Breaking Changes

- Make [`ETCDCTL_API=3 etcdctl` default](https://GDTS/utils/coreos/etcd/issues/9600).
...
- **Remove `etcd --ca-file` flag**, instead [use `etcd --trusted-ca-file`](https://GDTS/utils/coreos/etcd/pull/9470) (`etcd --ca-file` flag has been marked deprecated since v2.1).
- **Remove `etcd --peer-ca-file` flag**, instead [use `etcd --peer-trusted-ca-file`](https://GDTS/utils/coreos/etcd/pull/9470) (`etcd --peer-ca-file` flag has been marked deprecated since v2.1).
- **Remove `pkg/transport.TLSInfo.CAFile` field**, instead [use `pkg/transport.TLSInfo.TrustedCAFile`](https://GDTS/utils/coreos/etcd/pull/9470) (`CAFile` field has been marked deprecated since v2.1).
...
- Drop [ACIs from official release](https://GDTS/utils/coreos/etcd/pull/9059).
  - [AppC was officially suspended](https://github.com/appc/spec#-disclaimer-), as of late 2016.
  - [`acbuild`](https://github.com/containers/build#this-project-is-currently-unmaintained) is not maintained anymore.
  - `*.aci` files are not available from `v3.4` release.
- Exit on [empty hosts in advertise URLs](https://GDTS/utils/coreos/etcd/pull/8786).
...
- Exit on [shadowed environment variables](https://GDTS/utils/coreos/etcd/pull/9382).
...
- Change [`etcdserverpb.AuthRoleRevokePermissionRequest/key,range_end` fields type from `string` to `bytes`](https://GDTS/utils/coreos/etcd/pull/9433).
- Rename [`etcd_debugging_mvcc_db_total_size_in_bytes` Prometheus metric to `etcd_mvcc_db_total_size_in_bytes`](https://GDTS/utils/coreos/etcd/pull/9819).
...
- Rename `embed.Config.SnapCount` field to [`embed.Config.SnapshotCount`](https://GDTS/utils/coreos/etcd/pull/9745), to be consistent with the flag name `etcd --snapshot-count`.
- Change [`embed.Config.CorsInfo` in `*cors.CORSInfo` type to `embed.Config.CORS` in `map[string]struct{}` type](https://GDTS/utils/coreos/etcd/pull/9490).
- Remove [`embed.Config.SetupLogging`](https://GDTS/utils/coreos/etcd/pull/9572).
  - Now logger is set up automatically based on [`embed.Config.Logger`, `embed.Config.LogOutputs`, `embed.Config.Debug` fields](https://GDTS/utils/coreos/etcd/pull/9572).
- Rename [`etcd --log-output` to `etcd --log-outputs`](https://GDTS/utils/coreos/etcd/pull/9624) to support multiple log outputs.
  - **`etcd --log-output`** will be deprecated in v3.5.
- Rename [**`embed.Config.LogOutput`** to **`embed.Config.LogOutputs`**](https://GDTS/utils/coreos/etcd/pull/9624) to support multiple log outputs.
- Change [**`embed.Config.LogOutputs`** type from `string` to `[]string`](https://GDTS/utils/coreos/etcd/pull/9579) to support multiple log outputs.
...
- Change v3 `etcdctl snapshot` exit codes with [`snapshot` package](https://GDTS/utils/coreos/etcd/pull/9118/commits/df689f4280e1cce4b9d61300be13ca604d41670a).
  - Exit on error with exit code 1 (no more exit code 5 or 6 on `snapshot save/restore` commands).
- Migrate dependency management tool from `glide` to [`golang/dep`](https://GDTS/utils/coreos/etcd/pull/9155).
  - <= 3.3 puts `vendor` directory under `cmd/vendor` directory to [prevent conflicting transitive dependencies](https://GDTS/utils/coreos/etcd/issues/4913).
  - 3.4 moves `cmd/vendor` directory to `vendor` at repository root.
  - Remove recursive symlinks in `cmd` directory.
  - Now `go get/install/build` on `etcd` packages (e.g. `clientv3`, `tools/benchmark`) enforce builds with etcd `vendor` directory.
- Replace [gRPC gateway](https://github.com/grpc-ecosystem/grpc-gateway) endpoint `/v3beta` with [`/v3`](https://GDTS/utils/coreos/etcd/pull/9298).
  - Deprecated [`/v3alpha`](https://GDTS/utils/coreos/etcd/pull/9298).
  - To deprecate [`/v3beta`](https://GDTS/utils/coreos/etcd/issues/9189) in v3.5.
...
- Change [`wal` package function signatures](https://GDTS/utils/coreos/etcd/pull/9572) to support [structured logger and logging to file](https://GDTS/utils/coreos/etcd/issues/9438) in server-side.
...
- Remove [`pkg/cors` package](https://GDTS/utils/coreos/etcd/pull/9490).
...
- Move internal packages to `etcdserver`.
  - `"GDTS/utils/coreos/etcd/alarm"` to `"GDTS/utils/coreos/etcd/etcdserver/api/v3alarm"`.
  - `"GDTS/utils/coreos/etcd/compactor"` to `"GDTS/utils/coreos/etcd/etcdserver/api/v3compactor"`.
  - `"GDTS/utils/coreos/etcd/discovery"` to `"GDTS/utils/coreos/etcd/etcdserver/api/v2discovery"`.
  - `"GDTS/utils/coreos/etcd/etcdserver/auth"` to `"GDTS/utils/coreos/etcd/etcdserver/api/v2auth"`.
  - `"GDTS/utils/coreos/etcd/etcdserver/membership"` to `"GDTS/utils/coreos/etcd/etcdserver/api/membership"`.
  - `"GDTS/utils/coreos/etcd/etcdserver/stats"` to `"GDTS/utils/coreos/etcd/etcdserver/api/v2stats"`.
  - `"GDTS/utils/coreos/etcd/error"` to `"GDTS/utils/coreos/etcd/etcdserver/api/v2error"`.
  - `"GDTS/utils/coreos/etcd/rafthttp"` to `"GDTS/utils/coreos/etcd/etcdserver/api/rafthttp"`.
  - `"GDTS/utils/coreos/etcd/snap"` to `"GDTS/utils/coreos/etcd/etcdserver/api/snap"`.
  - `"GDTS/utils/coreos/etcd/store"` to `"GDTS/utils/coreos/etcd/etcdserver/api/v2store"`.
...
- Upgrade [`github.com/ugorji/go/codec`](https://github.com/ugorji/go/releases) to [**`v1.1.1`**](https://github.com/ugorji/go/releases/tag/v1.1.1), and [regenerate v2 `client`](https://GDTS/utils/coreos/etcd/pull/9494).
...
- Add [`etcd_network_active_peers`](https://GDTS/utils/coreos/etcd/pull/9762) Prometheus metric.
...
- Add [`etcd_network_disconnected_peers_total`](https://GDTS/utils/coreos/etcd/pull/9762) Prometheus metric.
...
- Add [`etcd_network_server_stream_failures_total`](https://GDTS/utils/coreos/etcd/pull/9760) Prometheus metric.
...
- Increase [`etcd_network_peer_round_trip_time_seconds`](https://GDTS/utils/coreos/etcd/pull/9762) Prometheus metric histogram upper-bound.
...
- Add [`etcd_server_heartbeat_send_failures_total`](https://GDTS/utils/coreos/etcd/pull/9761) Prometheus metric.
- Add [`etcd_server_slow_apply_total`](https://GDTS/utils/coreos/etcd/pull/9761) Prometheus metric.
...
- Add [`etcd_mvcc_db_total_size_in_bytes`](https://GDTS/utils/coreos/etcd/pull/9819) Prometheus metric.
  - Renamed from [`etcd_debugging_mvcc_db_total_size_in_bytes`](https://GDTS/utils/coreos/etcd/pull/9819).
- Add [`etcd_mvcc_db_total_size_in_use_in_bytes`](https://GDTS/utils/coreos/etcd/pull/9256) Prometheus metric.
  ...
  - `etcd_mvcc_db_total_size_in_use_in_bytes 16384` means future DB size if defragment operation is complete.
  - `etcd_mvcc_db_total_size_in_bytes - etcd_mvcc_db_total_size_in_use_in_bytes` is the number of bytes that can be saved on disk with defragment operation.
- Add [`etcd_snap_fsync_duration_seconds`](https://GDTS/utils/coreos/etcd/pull/9762) Prometheus metric.
- Add [`etcd_disk_backend_defrag_duration_seconds`](https://GDTS/utils/coreos/etcd/pull/9761) Prometheus metric.
- Add [`etcd_mvcc_hash_duration_seconds`](https://GDTS/utils/coreos/etcd/pull/9761) Prometheus metric.
- Add [`etcd_mvcc_hash_rev_duration_seconds`](https://GDTS/utils/coreos/etcd/pull/9761) Prometheus metric.
- Add [`etcd_debugging_disk_backend_commit_rebalance_duration_seconds`](https://GDTS/utils/coreos/etcd/pull/9834) Prometheus metric.
- Add [`etcd_debugging_disk_backend_commit_spill_duration_seconds`](https://GDTS/utils/coreos/etcd/pull/9834) Prometheus metric.
- Add [`etcd_debugging_disk_backend_commit_write_duration_seconds`](https://GDTS/utils/coreos/etcd/pull/9834) Prometheus metric.
- Add [`etcd_debugging_lease_granted_total`](https://GDTS/utils/coreos/etcd/pull/9778) Prometheus metric.
- Add [`etcd_debugging_lease_revoked_total`](https://GDTS/utils/coreos/etcd/pull/9778) Prometheus metric.
- Add [`etcd_debugging_lease_renewed_total`](https://GDTS/utils/coreos/etcd/pull/9778) Prometheus metric.
- Add [`etcd_debugging_lease_ttl_total`](https://GDTS/utils/coreos/etcd/pull/9778) Prometheus metric.
- Increase [`etcd_debugging_mvcc_index_compaction_pause_duration_milliseconds`](https://GDTS/utils/coreos/etcd/pull/9762) Prometheus metric histogram upper-bound.
...
- Fix missing [`etcd_network_peer_sent_failures_total`](https://GDTS/utils/coreos/etcd/pull/9437) Prometheus metric count.
...
- Add [`etcd --host-whitelist`](https://GDTS/utils/coreos/etcd/pull/9372) flag, [`etcdserver.Config.HostWhitelist`](https://GDTS/utils/coreos/etcd/pull/9372), and [`embed.Config.HostWhitelist`](https://GDTS/utils/coreos/etcd/pull/9372), to prevent ["DNS Rebinding"](https://en.wikipedia.org/wiki/DNS_rebinding) attack.
...
- Support [`etcd --cors`](https://GDTS/utils/coreos/etcd/pull/9490) in v3 HTTP requests (gRPC gateway).
- Support [`ttl` field for `etcd` Authentication JWT token](https://GDTS/utils/coreos/etcd/pull/8302).
  - e.g. `etcd --auth-token jwt,pub-key=<pub key path>,priv-key=<priv key path>,sign-method=<sign method>,ttl=5m`.
- Allow empty token provider in [`etcdserver.ServerConfig.AuthToken`](https://GDTS/utils/coreos/etcd/pull/9369).
...
- Add [`etcd --pre-vote`](https://GDTS/utils/coreos/etcd/pull/9352) flag to enable to run an additional Raft election phase.
  - For instance, a flaky(or rejoining) member may drop in and out, and start campaign. This member will end up with a higher term, and ignore all incoming messages with lower term. In this case, a new leader eventually need to get elected, thus disruptive to cluster availability. Raft implements Pre-Vote phase to prevent this kind of disruptions. If enabled, Raft runs an additional phase of election to check if pre-candidate can get enough votes to win an election.
...
- [`etcd --enable-v2v3`](TODO) flag is now stable.
  ...
  - Added [more v2v3 integration tests](https://GDTS/utils/coreos/etcd/pull/9634).
  ...
  - Automatically [create parent directory if it does not exist](https://GDTS/utils/coreos/etcd/pull/9626) (fix [issue#9609](https://GDTS/utils/coreos/etcd/issues/9609)).
  - v4.0 will configure `etcd --enable-v2=true --enable-v2v3=/aaa` to enable v2 API server that is backed by **v3 storage**.
- Add [`etcd --discovery-srv-name`](https://GDTS/utils/coreos/etcd/pull/8690) flag to support custom DNS SRV name with discovery.
...
- Rename [`etcd --log-output` to `etcd --log-outputs`](https://GDTS/utils/coreos/etcd/pull/9624) to support multiple log outputs.
  - **`etcd --log-output` will be deprecated in v3.5**.
- Add [`etcd --logger`](https://GDTS/utils/coreos/etcd/pull/9572) flag to support [structured logger and multiple log outputs](https://GDTS/utils/coreos/etcd/issues/9438) in server-side.
  ...
  - `etcd --logger=zap --log-outputs=default` is not supported when `etcd --logger=zap`.
    ...
    - However, even with PPID 1, it can fail to dial systemd journal (e.g. run embedded etcd with Docker container). Then, [every single log write will fail](https://GDTS/utils/coreos/etcd/pull/9729) and fall back to `os.Stderr`, which is inefficient.
  ...
  - `etcd --logger=zap --log-outputs=a.log,b.log,c.log,stdout` [writes server logs to multiple files `a.log`, `b.log` and `c.log` at the same time](https://GDTS/utils/coreos/etcd/pull/9579) and outputs to `os.Stderr`, in [JSON-encoded format](https://godoc.org/go.uber.org/zap#NewProductionEncoderConfig).
...
- Allow [non-WAL files in `etcd --wal-dir` directory](https://GDTS/utils/coreos/etcd/pull/9743).
  - Previously, existing files such as [`lost+found`](https://GDTS/utils/coreos/etcd/issues/7287) in WAL directory prevent etcd server boot.
  - Now, WAL directory that contains only `lost+found` or a file that's not suffixed with `.wal` is considered non-initialized.

### API

- Add [`snapshot`](https://GDTS/utils/coreos/etcd/pull/9118) package for snapshot restore/save operations (see [`godoc.org/github.com/etcd/clientv3/snapshot`](https://godoc.org/GDTS/utils/coreos/etcd/clientv3/snapshot) for more).
- Add [`watch_id` field to `etcdserverpb.WatchCreateRequest`](https://GDTS/utils/coreos/etcd/pull/9065) to allow user-provided watch ID to `mvcc`.
  - Corresponding `watch_id` is returned via `etcdserverpb.WatchResponse`, if any.
- Add [`fragment` field to `etcdserverpb.WatchCreateRequest`](https://GDTS/utils/coreos/etcd/pull/9291) to request etcd server to [split watch events](https://GDTS/utils/coreos/etcd/issues/9294) when the total size of events exceeds `etcd --max-request-bytes` flag value plus gRPC-overhead 512 bytes.
...
- Add [`raftAppliedIndex` field to `etcdserverpb.StatusResponse`](https://GDTS/utils/coreos/etcd/pull/9176) for current Raft applied index.
- Add [`errors` field to `etcdserverpb.StatusResponse`](https://GDTS/utils/coreos/etcd/pull/9206) for server-side error.
  - e.g. `"etcdserver: no leader", "NOSPACE", "CORRUPT"`
- Add [`dbSizeInUse` field to `etcdserverpb.StatusResponse`](https://GDTS/utils/coreos/etcd/pull/9256) for actual DB size after compaction.
- Add [`WatchRequest.WatchProgressRequest`](https://GDTS/utils/coreos/etcd/pull/9869).
...
- Add [`embed.Config.CipherSuites`](https://GDTS/utils/coreos/etcd/pull/9801) to specify a list of supported cipher suites for TLS handshake between client/server  and peers.
  - If empty, Go auto-populates the list.
...
- Define [`embed.CompactorModePeriodic`](https://godoc.org/GDTS/utils/coreos/etcd/embed#pkg-variables) for `compactor.ModePeriodic`.
- Define [`embed.CompactorModeRevision`](https://godoc.org/GDTS/utils/coreos/etcd/embed#pkg-variables) for `compactor.ModeRevision`.
- Change [`embed.Config.CorsInfo` in `*cors.CORSInfo` type to `embed.Config.CORS` in `map[string]struct{}` type](https://GDTS/utils/coreos/etcd/pull/9490).
- Remove [`embed.Config.SetupLogging`](https://GDTS/utils/coreos/etcd/pull/9572).
  - Now logger is set up automatically based on [`embed.Config.Logger`, `embed.Config.LogOutputs`, `embed.Config.Debug` fields](https://GDTS/utils/coreos/etcd/pull/9572).
- Add [`embed.Config.Logger`](https://GDTS/utils/coreos/etcd/pull/9518) to support [structured logger `zap`](https://github.com/uber-go/zap) in server-side.
- Rename `embed.Config.SnapCount` field to [`embed.Config.SnapshotCount`](https://GDTS/utils/coreos/etcd/pull/9745), to be consistent with the flag name `etcd --snapshot-count`.
- Rename [**`embed.Config.LogOutput`** to **`embed.Config.LogOutputs`**](https://GDTS/utils/coreos/etcd/pull/9624) to support multiple log outputs.
- Change [**`embed.Config.LogOutputs`** type from `string` to `[]string`](https://GDTS/utils/coreos/etcd/pull/9579) to support multiple log outputs.
...
- Add [`CLUSTER_DEBUG` to enable test cluster logging](https://GDTS/utils/coreos/etcd/pull/9678).
  - Deprecated `capnslog` in integration tests.

### client v3

- Add [`WithFragment` `OpOption`](https://GDTS/utils/coreos/etcd/pull/9291) to support [watch events fragmentation](https://GDTS/utils/coreos/etcd/issues/9294) when the total size of events exceeds `etcd --max-request-bytes` flag value plus gRPC-overhead 512 bytes.
  ...
  - For example, watch response contains 10 events, where each event is 1 MiB. And server `etcd --max-request-bytes` flag value is 1 MiB. Then, server will send 10 separate fragmented events to the client.
  - For example, watch response contains 5 events, where each event is 2 MiB. And server `etcd --max-request-bytes` flag value is 1 MiB and `clientv3.Config.MaxCallRecvMsgSize` is 1 MiB. Then, server will try to send 5 separate fragmented events to the client, and the client will error with `"code = ResourceExhausted desc = grpc: received message larger than max (...)"`.
- Add [`Watcher.RequestProgress` method](https://GDTS/utils/coreos/etcd/pull/9869).
  - To manually trigger broadcasting watch progress event (empty watch response with latest header) to all associated watch streams.
  - Think of it as `WithProgressNotify` that can be triggered manually.
...
- Make [`ETCDCTL_API=3 etcdctl` default](https://GDTS/utils/coreos/etcd/issues/9600).
  - Now, `etcdctl set foo bar` must be `ETCDCTL_API=2 etcdctl set foo bar`.
  - Now, `ETCDCTL_API=3 etcdctl put foo bar` could be just `etcdctl put foo bar`.
- Add [`etcdctl --password`](https://GDTS/utils/coreos/etcd/pull/9730) flag.
  - To support [`:` character in user name](https://GDTS/utils/coreos/etcd/issues/9691).
  - e.g. `etcdctl --user user --password password get foo`
- Add [`etcdctl user add --new-user-password`](https://GDTS/utils/coreos/etcd/pull/9730) flag.
- Add [`etcdctl check datascale`](https://GDTS/utils/coreos/etcd/pull/9185) command.
- Add [`etcdctl check datascale --auto-compact, --auto-defrag`](https://GDTS/utils/coreos/etcd/pull/9351) flags.
- Add [`etcdctl check perf --auto-compact, --auto-defrag`](https://GDTS/utils/coreos/etcd/pull/9330) flags.
- Add [`etcdctl defrag --cluster`](https://GDTS/utils/coreos/etcd/pull/9390) flag.
- Add ["raft applied index" field to `endpoint status`](https://GDTS/utils/coreos/etcd/pull/9176).
- Add ["errors" field to `endpoint status`](https://GDTS/utils/coreos/etcd/pull/9206).
- Add [`etcdctl endpoint health --write-out` support](https://GDTS/utils/coreos/etcd/pull/9540).
  - Previously, [`etcdctl endpoint health --write-out json` did not work](https://GDTS/utils/coreos/etcd/issues/9532).
...
- Add [`progress` command to `etcdctl watch --interactive`](https://GDTS/utils/coreos/etcd/pull/9869).
  - To manually trigger broadcasting watch progress event (empty watch response with latest header) to all associated watch streams.
  - Think of it as `WithProgressNotify` that can be triggered manually.

### gRPC proxy

- Fix [etcd server panic from restore operation](https://GDTS/utils/coreos/etcd/pull/9775).
  - Let's assume that a watcher had been requested with a future revision X and sent to node A that became network-partitioned thereafter. Meanwhile, cluster makes progress. Then when the partition gets removed, the leader sends a snapshot to node A. Previously if the snapshot's latest revision is still lower than the watch revision X,  **etcd server panicked** during snapshot restore operation.
...
- Replace [gRPC gateway](https://github.com/grpc-ecosystem/grpc-gateway) endpoint `/v3beta` with [`/v3`](https://GDTS/utils/coreos/etcd/pull/9298).
  ...
  - To deprecate [`/v3beta`](https://GDTS/utils/coreos/etcd/issues/9189) in v3.5.
  - In v3.4, `curl -L http://localhost:2379/v3beta/kv/put -X POST -d '{"key": "Zm9v", "value": "YmFy"}'` still works as a fallback to `curl -L http://localhost:2379/v3/kv/put -X POST -d '{"key": "Zm9v", "value": "YmFy"}'`, but `curl -L http://localhost:2379/v3beta/kv/put -X POST -d '{"key": "Zm9v", "value": "YmFy"}'` won't work in v3.5. Use `curl -L http://localhost:2379/v3/kv/put -X POST -d '{"key": "Zm9v", "value": "YmFy"}'` instead.
- Add API endpoints [`/{v3beta,v3}/lease/leases, /{v3beta,v3}/lease/revoke, /{v3beta,v3}/lease/timetolive`](https://GDTS/utils/coreos/etcd/pull/9450).
  - To deprecate [`/{v3beta,v3}/kv/lease/leases, /{v3beta,v3}/kv/lease/revoke, /{v3beta,v3}/kv/lease/timetolive`](https://GDTS/utils/coreos/etcd/issues/9430) in v3.5.
- Support [`etcd --cors`](https://GDTS/utils/coreos/etcd/pull/9490) in v3 HTTP requests (gRPC gateway).

### Package `raft`

- Fix [deadlock during PreVote migration process](https://GDTS/utils/coreos/etcd/pull/8525).
- Add [`raft.ErrProposalDropped`](https://GDTS/utils/coreos/etcd/pull/9067).
  - Now [`(r *raft) Step` returns `raft.ErrProposalDropped`](https://GDTS/utils/coreos/etcd/pull/9137) if a proposal has been ignored.
  - e.g. a node is removed from cluster, or [`raftpb.MsgProp` arrives at current leader while there is an ongoing leadership transfer](https://GDTS/utils/coreos/etcd/issues/8975).
- Improve [Raft `becomeLeader` and `stepLeader`](https://GDTS/utils/coreos/etcd/pull/9073) by keeping track of latest `pb.EntryConfChange` index.
  - Previously record `pendingConf` boolean field scanning the entire tail of the log, which can delay hearbeat send.
- Fix [missing learner nodes on `(n *node) ApplyConfChange`](https://GDTS/utils/coreos/etcd/pull/9116).

### Tooling

- Add [`etcd-dump-logs --entry-type`](https://GDTS/utils/coreos/etcd/pull/9628) flag to support WAL log filtering by entry type.
- Add [`etcd-dump-logs --stream-decoder`](https://GDTS/utils/coreos/etcd/pull/9790) flag to support custom decoder.
