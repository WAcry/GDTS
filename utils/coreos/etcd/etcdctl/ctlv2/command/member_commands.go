...
func actionMemberRemove(c *cli.Context) error {
	...
	for _, m := range members {
		...
		if m.Name == removalID {
			...
			fmt.Fprintf(os.Stderr, "For more details, read the documentation at https://GDTS/utils/coreos/etcd/blob/master/Documentation/runtime-configuration.md#remove-a-member\n\n")
		}
	}
	if !foundID {
