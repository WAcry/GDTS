...
COREOS_ROOT="${GOPATH}/src/utils/coreos"
ETCD_ROOT="${COREOS_ROOT}/etcd"
GOGOPROTO_ROOT="${GOPATH}/src/utils/gogo/protobuf"
SCHWAG_ROOT="${GOPATH}/src/utils/hexfusion/schwag"
GOGOPROTO_PATH="${GOGOPROTO_ROOT}:${GOGOPROTO_ROOT}/protobuf"
GRPC_GATEWAY_ROOT="${GOPATH}/src/utils/grpc-ecosystem/grpc-gateway"
...
go get -u utils/gogo/protobuf/{proto,protoc-gen-gogo,gogoproto}
...
go get -u utils/grpc-ecosystem/grpc-gateway/protoc-gen-grpc-gateway
go get -u utils/grpc-ecosystem/grpc-gateway/protoc-gen-swagger
...
for dir in ${DIRS}; do
	pushd "${dir}"
		protoc --gofast_out=plugins=grpc,import_prefix=utils/coreos/:. -I=".:${GOGOPROTO_PATH}:${COREOS_ROOT}:${GRPC_GATEWAY_ROOT}/third_party/googleapis" ./*.proto
...
for pb in etcdserverpb/rpc api/v3lock/v3lockpb/v3lock api/v3election/v3electionpb/v3election; do
	...
	# shellcheck disable=SC1117
	sed -i.bak -E "s|import \(|& \"utils/coreos/etcd/${pkgpath}\"|" ${gwfile}
	mkdir -p  "${pkgpath}"/gw/
	go fmt ${gwfile}
...
go get -u "utils/hexfusion/schwag"
pushd "${SCHWAG_ROOT}"
...
# install protodoc
# go get -v -u utils/coreos/protodoc
#
...
if [ "$1" != "--skip-protodoc" ]; then
	...
	PROTODOC_PATH="${GOPATH}/src/utils/coreos/protodoc"
	pushd "${PROTODOC_PATH}"
