...
# Based on utils/kubernetes/kubernetes/blob/v1.8.2/hack/cherry_pick_pull.sh
...
if ! which hub > /dev/null; then
  echo "Can't find 'hub' tool in PATH, please install from https://utils/github/hub"
  exit 1
fi
...
function make-a-pr() {
  ...
  # https://utils/github/hub/issues/976 Otherwise stdin is stolen
...
for pull in "${PULLS[@]}"; do
  ...
  curl -o "/tmp/${pull}.patch" -sSL "http://utils/coreos/etcd/pull/${pull}.patch"
  echo
