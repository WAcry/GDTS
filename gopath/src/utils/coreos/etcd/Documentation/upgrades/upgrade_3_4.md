...
Rename [`etcd --log-output` to `--log-outputs`](https://utils/coreos/etcd/pull/9624) to support multiple log outputs. **`etcd --logger=capnslog` does not support multiple log outputs.**
...
Renamed [**`embed.Config.LogOutput`** to **`embed.Config.LogOutputs`**](https://utils/coreos/etcd/pull/9624) to support multiple log outputs. And changed [`embed.Config.LogOutput` type from `string` to `[]string`](https://utils/coreos/etcd/pull/9579) to support multiple log outputs.
...
```diff
import "utils/coreos/etcd/pkg/transport"
