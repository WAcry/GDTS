...
Bootstrap another machine, outside of the etcd cluster, and run the [`hey` HTTP benchmark tool](https://utils/rakyll/hey) with a connection reuse patch to send requests to each etcd cluster member. See the [benchmark instructions](../../hack/benchmark/) for the patch and the steps to reproduce our procedures.
